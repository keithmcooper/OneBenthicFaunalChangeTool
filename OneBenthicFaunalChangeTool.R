###########################################################################################
####                          ONE BENTHIC FAUNAL CHANGE TOOL                         ####
###########################################################################################
...
library(vegan)
library(config)
library(shinycssloaders) #https://stackoverflow.com/questions/49488228/how-to-show-spinning-wheel-or-busy-icon-while-waiting-in-shiny
library(shinybusy)
library(ggh4x)
#library(geojsonio)
...
ui <- fluidPage(
  ...
  # add_busy_spinner(spin = "fading-circle",position = "top-right",margins = c(25, 20),color = "#FFFFFF",timeout = 300,height = "60px",width = "60px"),##FFFFFF #0D4581 margins 2nd fig from right
  #### TITLE PANEL ####
  titlePanel(title=div(img(src="onebenthic.gif",tags$b(" OneBenthic"),"Faunal Change Tool",height = 70, width = 170),style='background-color:#B4C7E7;padding-right: 50px')),
  #__________________________________________________________________________________________  
  fluidRow(
    #### SELECTIONS ####
    column(2,
    ...
    column(6,style='border-left: 1px solid grey',
           tabsetPanel(
             ...
             tabPanel(tags$b("Results"),
                      tabsetPanel(
                        #__________________________________________________________________________________________%>% withSpinner(color="#B4C7E7")
                        #### RESULTS TAB: SITE ####
                        tabPanel("Site",
                                 tabsetPanel(
                                   tabPanel(em("Boxplot"),withSpinner(plotOutput(outputId = "site.line", height = 740),color='#B4C7E7')),
                                   tabPanel(em("Means"),br(),div(withSpinner(DT::dataTableOutput("unimeanssite"),color='#B4C7E7'),style = 'font-size:85%')),
                                   tabPanel(em("Anova"),br(),div(withSpinner(DT::dataTableOutput("sedsumsite"),color='#B4C7E7'),style = 'font-size:85%')),
                        ...
                        tabPanel("Sub-region",
                                 tabsetPanel(
                                   tabPanel(em("Boxplot"),value= 'subregionTab',withSpinner(plotOutput(outputId = "subreg.line", height = 740),color='#B4C7E7')),
                                   tabPanel(em("Means"),value= 'subregionTab',br(),div(withSpinner(DT::dataTableOutput("unimeanssubreg"),color='#B4C7E7'),style = 'font-size:85%')),
                                   tabPanel(em("Anova"),value= 'subregionTab',br(),div(withSpinner(DT::dataTableOutput("sedsumsubreg"),color='#B4C7E7'),style = 'font-size:85%')),
                                   ...
                                   #####
                                   #,
                                   #tabPanel(em("Change"),value= 'subregionTab',plotOutput(outputId = "change.subreg", height = 740))
                                   ,id ="tabselected2"
                                   ####
                        ...
                        tabPanel("Region",
                                 tabsetPanel(
                                   tabPanel(em("Boxplot"),value = 'regionTab',br(),withSpinner(plotOutput(outputId = "reg.line", height = 350),color='#B4C7E7')),
                                   tabPanel(em("Means"),value = 'regionTab',br(),div(withSpinner(DT::dataTableOutput("unimeansreg"),color='#B4C7E7'),style = 'font-size:85%')),
                                   tabPanel(em("Anova"),value = 'regionTab',br(),div(withSpinner(DT::dataTableOutput("sedsumreg"),color='#B4C7E7'),style = 'font-size:85%')),
                                   tabPanel(em("nMDS 2D"),value = 'regionTab',br(),withSpinner(plotOutput(outputId = "nmds", height = 350),color='#B4C7E7')),#750
                                   ...
                                   #####
                                   #,
                                   #tabPanel(em("Change"),value = 'regionTab',br(),plotOutput(outputId = "change.reg", height = 740))
                                   , id ="tabselected"
                                   ####
             ...
             tabPanel(tags$b("About"),
                      tabsetPanel(
                        ...
                        tabPanel("Instructions",
                                 h4("App purpose"),"This app is designed to look for evidence of statistically significant changes in macrofaunal composition between baseline and monitoring surveys. The app can be used for monitoring of marine licences (e.g. marine aggregate, offshore wind, renewables), MPAs or R&D sites.",br(),
                                 h4("Data"),"The macrofaunal abundance data used in the app come from the ",tags$a(href="https://rconnect.cefas.co.uk/onebenthic_portal/", "OneBenthic "),"database. The app also uses spatial data polygons (see 'Map Layers' tab) to automatically assign samples to treatment groups (e.g. reference, licensed areas and secondary impact zones).",br(),h4("How it works"),"Select sector, baseline and monitoring surveys for comparison, and whether to use all or paired samples only (i.e. stations with both a baseline and a monitoring sample). Selected samples are shown in the map. For the marine aggregates sector (AGG), users must also select an RSMP survey array. To find surveys for comparison, turn on sample locations (see map 'samples' checkbox) and use the 'Draw a rectangle' tool to highlight an area of interest. Note, a full list of surveys associated with each of the aggregate industry's RSMP arrays can be seen in 'samples' tab of the OneBenthic Baseline tool. Results will appear in the 'Search' tab. Selected data and assigned treatment groups can be viewed under the 'Data' tab.","Data are analysed in various ways, with results available, where relevant, at different spatial scales (see 'Results' tab).",br(),br(),
                                 "The",tags$b("Boxplot"),"tab provides a graphical representation of differences in mean values of univariate summary metrics (taxon richness and abundance) between baseline and monitoring surveys by treatment category.",br(),br(),
                                 "The",tags$b("Anova")," tab provides results of an Analysis of Variance, a statistical test for difference in the mean of groups (e.g. baseline vs monitoring).",br(),br(),
                                 "The", tags$b("MDS")," tab shows a series of non-metric multidimensional scaling ordination (nMDS) plots based on Bray Curtis dissimilarity (4th root transformed macrofaunal abundance data). Each dot represents a sample, with positions reflecting similarity/dis-similarity in terms of macrofaunal composition. The stress value provides a measure of how well the 2-d plot represents the multidimensional data (stress values of <0.1 are considered good).",br(),br(),
                                 "The", tags$b("Anosim")," test looks for evidence of statistically significant differences between the two groups of samples (i.e. baseline vs monitoring). Test outputs include R and p-values. The R value indicates the size of the difference between the two groups, with 0 indicating no difference and 1 a large difference. A p-value of <0.05 indicates that results are statistically significant. Both R and p-values should be considered when interpreting test results. Note that where there are large numbers of samples it may be possible to find statistically significant, yet very small differences between the groups. It is therefore important to consider the effect size (R value). For this reason we generally consider differences to only be of interest where p<0.05 and R>0.1.","Anosim test outputs include a column for 'interpretation', following criteria in ",tags$a(href="https://www.researchgate.net/post/Which-R-value-is-considered-to-show-a-strong-difference-between-the-groups-in-ANOSIM/54ff36e4d2fd64c47b8b45a7/citation/download", "Goss-Souza (2015).")," Press button to see results displayed in the map (only relevant to treatment polygons). ANOSIM tests are performed using the R",tags$a(href="
https://CRAN.R-project.org/package=vegan", "vegan"),"package. Where ANOSIM finds a meaningful difference (i.e. p<0.05, R>0.1), relevant data are carried forward for a SIMPER test to identify which taxa are responsible for the differences.",br(),br(),
                                 "The",tags$b("Simper")," test is carried out using the 'simper' function (vegan) based on Bray-Curtis dissimilarities."," Note that analyses only run after clicking on each tab - please be patient.","You can change the R value cut-off for SIMPER analysis here:",numericInput("num", h4("",style="color:#808080"), value = 0.1, min = 0, max = 1, step=0.01) ,
...
server <- function(input, output, session) {
  ...
  observe({
    #req(input$programmeInput)
    if (input$programmeInput ==""|input$programmeInput =="SAC"|input$programmeInput =="MCZ"|input$programmeInput =="R&D"|input$programmeInput =="OWF"|input$programmeInput =="NONE"){
...
order by s.year desc, st.stationcode asc; ",
    ...
    ## Remove juv records
    ## Change NA to '
    coord4$taxaqual_qualifier[is.na(coord4$taxaqual_qualifier)] <- ''
    
    ## Remove rows where taxaqual = 'J'
    coord5 <- coord4[coord4$taxaqual_qualifier != 'J',]
    #coord5 <- coord5[coord5$samplecode != 'BSSS105',] #  Remove an outlier from survey 'Bassurelle_MCZ_infauna'
    ## Only include benthic taxa
    coord6 <- coord5[coord5$include == TRUE,]
  ...
  data_wide <- reactive({
    reshape2::dcast(coord(), samplecode+stationcode+samplelong+ samplelat+surveyname ~ validname, value.var="abund",fun.aggregate = mean)
  ...
  long <- reactive({
    ...
    a <- subset( data_wide(),surveyname %in% input$baselineInput)#
    a$time <- "baseline"
    ## Change NA to zero (to allow dor rowsum calculation)
    a[, 6:length(a)][is.na(a[, 6:length(a)])] <- 0
  ...
  longmon <- reactive({
    b <- subset(data_wide(),surveyname %in% input$monitoringInput)#samplecode,stationcode,samplelong,samplelat,surveyname,wwacr,SUM(sv.percentage) 
    b$time <- "monitoring"
    ## Change NA to zero (to allow dor rowsum calculation)
    b[, 6:length(b)][is.na(b[, 6:length(b)])] <- 0
    #View(b)
    return(b)
  })
  ...
  output$widebas <- DT::renderDataTable({
    ...
    long <- long()
    colnames(long)[1] <- "Sample Code"
    colnames(long)[2] <- "Station Code"
  ...
  output$widemon <- DT::renderDataTable({
    ...
    longmon <- longmon()
    colnames(longmon)[1] <- "Sample Code"
    colnames(longmon)[2] <- "Station Code"
    colnames(longmon)[3] <- "Long"
    colnames(longmon)[4] <- "Lat"
    colnames(longmon)[5] <- "Survey Name"
    #DT::datatable(longmon()[,c(1:5,12,9,11,7,8,10,6)],class = "display nowrap", options = list(pageLength = 19))%>%formatRound(columns=c("cG", "mG","fG","cS","mS","fS","SC"), digits=1) #removing [,1:11] will get rid of error message?????
    DT::datatable(longmon[,1:length(longmon)-1],class = "display nowrap",options = list(scrollX = TRUE,pageLength = 19,
                                                                                        language = list(zeroRecords = "Data not publicly available")
  ...
  output$wideall <- DT::renderDataTable({
    longalltest <- longalltest()
    colnames(longalltest)[1] <- "Sample Code"
    colnames(longalltest)[2] <- "Station Code"
    colnames(longalltest)[3] <- "Long"
    colnames(longalltest)[4] <- "Lat"
    colnames(longalltest)[5] <- "Survey Name"
    colnames(longalltest)[length(longalltest)] <- "Time"
    ...
    DT::datatable(longalltest,class = "display nowrap",options = list(scrollX = TRUE,pageLength = 19,
                                                                      language = list(zeroRecords = "Data not publicly available")
  ...
  stations <- reactive({
    ...
    stations_in_piz <-  stations_in_piz[,c(1:as.numeric(length(stations_in_piz)-3),as.numeric(length(stations_in_piz)-1):length(stations_in_piz))]
    
    #colnames(stations_in_piz)[14] <- "area"
    colnames(stations_in_piz)[as.numeric(length(stations_in_piz)-1)] <- "area"
    ...
    stations_in_siz <-  stations_in_siz[,c(1:as.numeric(length(stations_in_siz)-3),as.numeric(length(stations_in_siz)-1):length(stations_in_siz))]
    #colnames(stations_in_siz)[14] <- "region"
    colnames(stations_in_siz)[as.numeric(length(stations_in_siz)-1)] <- "area"
    ...
    #View(stations_in_siz)
    #__________________________________________________________________________________________         
    ## CONTEXT
    ## Where no data create an empty object. If remaining samples are not in SIZ then they must be context
    #if(countsiz==countsiz.na){
  ...
  output$refst <- DT::renderDataTable({
    ...
    stations <- stations()
    colnames(stations)[1] <- "Sample Code"
    colnames(stations)[2] <- "Station Code"
    colnames(stations)[3] <- "Long"
    colnames(stations)[4] <- "Lat"
    colnames(stations)[5] <- "Survey Name"
    colnames(stations)[length(stations)] <- "Treatment2"
    colnames(stations)[length(stations)-1] <- "Treatment"
    colnames(stations)[length(stations)-2] <- "Area"
    colnames(stations)[length(stations)-3] <- "Time"
    ...
    # DT::datatable(stations()[,c(1:5,stations()[as.numeric(length(stations)-3):length(stations)])])
    DT::datatable(stations[,c(1:5,as.numeric(length(stations)-3):length(stations))],class = "display nowrap",options = list(scrollX = TRUE,pageLength = 19,
                                                                                                                            language = list(zeroRecords = "Data not publicly available")
  ...
  #__________________________________________________________________________________________
  #### SITE: UNI METRIC MEANS TABLE ####     
  output$unimeanssite <- DT::renderDataTable({
    adata <- stations.site()[,c(as.numeric(ncol(stations.site())-1),as.numeric(ncol(stations.site())-4),5:as.numeric(ncol(stations.site())-5))]
    #View(adata)
    
    ## Change order of time variable
    #str(adata)
    adata$time <- factor(adata$time, levels=c("baseline","monitoring"))
    
    ## Call library
    library(vegan)
    ## Calculate univariate summary measures based on faunal abundance data in df 'data5'
    #data$richness = specnumber(data[,3:7]) # Species Richness(S)
    adata$richness = specnumber(adata[,3:as.numeric(length(adata))]) # Species Richness(S)
    adata$abundance = rowSums(adata[,3:as.numeric(length(adata))]) # S
    #head(data)
    
    ## Get data (rich, abund and factors) into long format
    library(tidyr)
    adata2 <- adata[,c(1:2,as.numeric(length(adata)-1):as.numeric(length(adata)))]
    #View(adata2)
    adata3 <- gather(adata2, uni, value, richness:abundance, factor_key=TRUE)
    # View(adata3)
    
    ## Calculate means for each metrics by treatment group
    adata4 <- adata3%>%group_by(treatment2, time, uni)%>%summarise(
      count=n(),mean = mean(value), sd=sd(value))
    
    ## Update col names
    colnames(adata4) <- c("Treatment","Time","Metric","Count","Mean","Std Dev")
    
    ## Change order of rows so baseline and monitoring next to each other for each metric
    adata5 <- adata4[order(adata4$Treatment,adata4$Metric),]
    
    ## Reorder cols
    adata6 <- adata5[,c(1,3,2,4,5,6)]
    
    #DT::datatable(adata4)
    DT::datatable(adata6, options = list(pageLength = 19))%>%formatRound(columns=c("Mean","Std Dev"), digits=1)
  })
  
  #__________________________________________________________________________________________
  #### SUB-REG: UNI METRIC MEANS TABLE ####     
  output$unimeansreg <- DT::renderDataTable({
    
    adata <- stations.subreg()[,c(as.numeric(ncol(stations.subreg())-1),as.numeric(ncol(stations.subreg())-4),5:as.numeric(ncol(stations.subreg())-5))]#treatment 2, time, faunal variables
    #View(adata)
    ...
    ## Calculate univariate summary measures based on faunal abundance data in df 'data5'
    #data$richness = specnumber(data[,3:7]) # Species Richness(S)
    adata$richness = specnumber(adata[,3:as.numeric(length(adata))]) # Richness(S)
    adata$abundance = rowSums(adata[,3:as.numeric(length(adata))]) # Abundance
    #head(data)
    
    ## Get data (rich, abund and factors) into long format
    library(tidyr)
    adata2 <- adata[,c(1:2,as.numeric(length(adata)-1):as.numeric(length(adata)))]# treatment2, time, richness, abundance
    #View(adata2)
    adata3 <- gather(adata2, uni, value, richness:abundance, factor_key=TRUE)# treatment2, time, uni(metric), value
    #View(adata3)
    
    ## Calculate means for each metrics by treatment group
    adata4 <- adata3%>%group_by(treatment2, time, uni)%>%summarise(count=n(),
  ...
  #__________________________________________________________________________________________
  #### REG: UNI METRIC MEANS TABLE ####     
  output$unimeansreg <- DT::renderDataTable({
    
    adata <- stations.reg()[,c(as.numeric(ncol(stations.reg())-1),as.numeric(ncol(stations.reg())-4),5:as.numeric(ncol(stations.reg())-5))]#treatment 2, time, faunal variables
    #View(adata)
  ...
  output$nmds <- renderPlot({
    ...
    ## Separate out sieve data for ordination. Make sure values are numeric
    #data2 = as.data.frame(sapply(data[,c(11,8,10,6,7,9,5)], as.numeric))
    data2 = as.data.frame(sapply(data[,5:as.numeric(length(data)-5)], as.numeric))
    
    #browser()
    ## Separate out env data (Wentworth - for overlay) for ordination. Make sure values are numeric
    #env = as.data.frame(sapply(data[,c(11,8,10,6,7,9,5)], as.numeric))
    #%#env = as.data.frame(sapply(data[,5:as.numeric(length(data)-5)], as.numeric))
    ...
    set.seed(123)
    # ord <- metaMDS(data2,distance="bray",k = 2)
    ord <- metaMDS(data2t,distance="bray",autotransform = FALSE,k = 2)
    ## Stress
  ...
  output$nmds3dsite <- renderPlotly({
    ...
    ## Perform the NMDS ordination 
    set.seed(123)
    ord <- metaMDS(data2t,distance="bray",autotransform = FALSE,k = 3)
  ...
  anosim.reg <- reactive({
    ...
    for (j in 1:nsites) {
      ...
      #site.use = data[use,c(560,5:559)]
      site.use = data[use,c(length(data)-4,5:as.numeric(length(data)-5))]
      time.use = site.use$time
      
      
      #matrix.use = site.use[,2:8]
      #matrix.use = site.use[,2:556]
      matrix.use = site.use[,2:length(site.use)]# works for anglian
      
      ## Identify columns (taxa) with zero abundance
      i <- (colSums(matrix.use, na.rm=T) != 0) # T if colSum is not 0, F otherwise
      
      ## Drop cols with zero abund
      matrix.use.nonzero <- matrix.use[, i] # all the non-zero columns
      
      ## Transform the data (fourth-root transformation)
      matrix.use.nonzero.t=matrix.use.nonzero^(0.25)
      
      ## ANOSIM test
      # anos = anosim(matrix.use, grouping = time.use)
      anos = anosim(matrix.use.nonzero.t, grouping = time.use,distance = "bray")
  ...
  anosim.site <- reactive({
    ...
    for (j in 1:nsites) {
      ...
      ## Get the particle size matrix and the grouping vector (time and then variables)
      #site.use = data[use,c(12,5:11)]
      #site.use = data[use,c(616,5:615)]# this works for anglian
      
      ######################
      #[,5:as.numeric(length(data)-5)]
      site.use = data[use,c(length(data)-4,5:as.numeric(length(data)-5))]
      time.use = site.use$time
      #matrix.use = site.use[,2:8]
      #matrix.use = site.use[,2:612]# works for anglian
      matrix.use = site.use[,2:length(site.use)]# works for anglian
      ##########
      ## Identify columns (taxa) with zero abundance
      i <- (colSums(matrix.use, na.rm=T) != 0) # T if colSum is not 0, F otherwise
  ...
  simper.reg <- reactive({
    ...
    ## Start with data in object 'stations4' from STEP 10.
    data <- stations.reg()
    data$treatment3 <- as.factor(data$treatment3)
    
    ## Seperate out sieve data for ordination. Make sure values are numeric
    #data2 = as.data.frame(sapply(data[,c(11,8,10,6,7,9,5)], as.numeric))
    data2 = as.data.frame(sapply(data[,5:as.numeric(ncol(stations.reg())-5)], as.numeric))
    ...
    sim <- simper(data2,  data$treatment3 , permutations = 99)
    #browser()
    simsum <- summary(sim)
    top3<-lapply(simsum, `[`,1:15,)#get top 3 contributors 
    ...
    ## Update column names
    colnames(simperdf6) <-c("Site","Taxon","Baseline","Monitoring","Diff","CumSum") 
  ...
  simper.subreg <- reactive({
    ...
    ## Seperate out sieve data for ordination. Make sure values are numeric
    #data2 = as.data.frame(sapply(data[,c(11,8,10,6,7,9,5)], as.numeric))
    data2 = as.data.frame(sapply(data[,5:as.numeric(ncol(data)-5)], as.numeric))
  ...
  output$simpertab.reg <- DT::renderDataTable({
    DT::datatable(simper.reg(), options = list(pageLength = 20,searching = FALSE,paging = FALSE,
                                               language = list(
                                                 zeroRecords = "Not relevant given Anosim found no R values > than set value.")))%>%formatRound(columns=c('Baseline', 'Monitoring','Diff','CumSum'), digits=2)
  ...
  sed.sum.reg <- reactive({ 
    ...
    ## Define data types
    adata3$treatment2 <- factor(adata3$treatment2)
    adata3$uni <- factor(adata3$uni)
    adata3$value <- as.numeric(adata3$value) 
    adata3$time <- factor(adata3$time)
    
    ## Do ANOVA
    
    #https://stackoverflow.com/questions/39664640/extract-values-from-nested-list-of-summaryaov-into-a-dataframe
    
    library(broom)
    library(dplyr)
    
    summaries <- adata3 %>% group_by(uni,treatment2) %>% 
      do(tidy(aov(value ~ time, data = .)))# + Block
    
    ## Change from tibble to df
    summaries <- as.data.frame(summaries,na.rm=True)
    
    ## Remove NAs
    #summaries[is.na(summaries)] <- ""  
    
    ## Add column for 'difference'
    summaries$interpretation <-summaries$p.value
    
    ## Now add interpretation
    summaries$interpretation[summaries$interpretation>0.05] <- "not statistically significant"
    summaries$interpretation[summaries$interpretation<0.05] <- "statistically significant"
  ...
  sed.sum.site <- reactive({ 
    ...
    ##################################
    ## Now remove samples where there are not baseline and monitoring samples
    # Create a factor for treatment2 and univariate metric
    adata3$x <- as.factor(paste(adata3$treatment2,adata3$uni))
    
    # filter out rows for treatments where there aren't baseline and monitoring samples
    library(dplyr)
    vec=c('baseline', 'monitoring')
    adata4 <-adata3 %>% 
      group_by(x) %>% 
      filter(all(vec %in% time))
    adata3 <- adata4[,1:4]
  ...
  #__________________________________________________________________________________________
  #### REGION:  ANOVA TABLE ####      WAS MEANS DATA
  ...
  output$sedsumreg <- DT::renderDataTable({
    ...
    DT::datatable(sed.sum.reg(),colnames=c("Metric","Treatment","Term","df","Sum Sq","Mean Sq","F value","Pr", "Interpretation"), options = list(pageLength = 12))%>%formatRound(columns=c('sumsq', 'meansq', 'statistic', 'p.value'), digits=3)%>%formatStyle('interpretation',backgroundColor=styleEqual(c("not statistically significant","statistically significant"),c('#2dc937','#cc3232')))
    
  })
  
  #__________________________________________________________________________________________
  #### SUBEGION: ANOVA TABLE ####      
  ...
  output$sedsumsubreg <- DT::renderDataTable({
    ...
    DT::datatable(sed.sum.subreg(),colnames=c("Metric","Treatment","Term","df","Sum Sq","Mean Sq","F value","Pr", "Interpretation"), options = list(pageLength = 12))%>%formatRound(columns=c('sumsq', 'meansq', 'statistic', 'p.value'), digits=3)%>%formatStyle('interpretation',backgroundColor=styleEqual(c("not statistically significant","statistically significant"),c('#2dc937','#cc3232')))
  ...
  output$sedsumsite <- DT::renderDataTable({
    ...
    #DT::datatable(sed.sum.site())
    DT::datatable(sed.sum.site(),colnames=c("Metric","Treatment","Term","df","Sum Sq","Mean Sq","F value","Pr", "Interpretation"), options = list(pageLength = 12))%>%formatRound(columns=c('sumsq', 'meansq', 'statistic', 'p.value'), digits=3)%>%formatStyle('interpretation',backgroundColor=styleEqual(c("not statistically significant","statistically significant"),c('#2dc937','#cc3232')))
    #DT::datatable(anosim.site(), colnames=c("Site","R","p","Interpretation"),options = list(pageLength = 15))%>%formatRound(columns=c('R', 'p'), digits=3)%>%formatStyle('interpretation',backgroundColor=styleEqual(c("highly different","different","different with some overlap","similar with some differences (or high overlap)","similar","no evidence of change (p>0.05)"),c('#cc3232','#db7b2b','#e7b416','#99c140','#2dc937','#2dc937')))
  })
  
  #__________________________________________________________________________________________
  #### REGION: BOX & WHISKER PLOTS #### was LINE
  
  output$reg.line <- renderPlot({
    ##adata <- stations.reg()[,c(15,12,11,8,10,6,7,9,5,1)]#treatment2, time, SC,fS,mS,cS,fG,mG,cG,stationcode 
    # adata <- stations.reg()[,c(619,617,5:616,1)]#treatment2, time, SC,fS,mS,cS,fG,mG,cG,stationcode 
    
    #View(stations.reg())
    #adata <- stations.reg()[,c(619,616,5:615)]# treatment2, time, faunal vars
    #adata <- stations.reg()[,c(as.numeric(ncol(stations.reg())-1),616,5:615)]
    adata <- stations.reg()[,c(as.numeric(ncol(stations.reg())-1),as.numeric(ncol(stations.reg())-4),5:as.numeric(ncol(stations.reg())-5))]#treatment 2, time, faunal variables
    #View(adata)
    ...
    ## Plots
    p <- ggplot()+
      #geom_boxplot(data=adata3,aes(x=time, y=richness,col=time, group=time),size = 1)+ #means 
      geom_boxplot(data=adata3,aes(x=time, y=value,col=time, group=time),size = 1)+ #means
      
      scale_colour_manual(values = c("blue","#00CCCC"))+
      labs(x="Time",y="Value")+
      #facet_wrap(~treatment2)+
      #facet_wrap(~uni)+#works
      #facet_grid(treatment2~uni, scales = "free_y")+
      #facet_grid(treatment2~uni,scales="free_y")+
      #ggh4x::facet_grid2(treatment2~uni, scales = "free_y", independent = "y")
      #facet_grid(treatment2~uni, scales = "free_y")+
      #facet_wrap(. ~ uni, scales="free_y")
      facet_grid2(treatment2~uni, scales = "free", independent = "all")
    theme(text = element_text(size=11),legend.title = element_blank(), 
          legend.text=element_text(size=11))#a 
    print(p)
    #View(adata3)
    #write.csv(adata(),"C:\\Users\\KMC00\\OneDrive - CEFAS\\R_PROJECTS\\OneBenthicFaunalChangeTool\\DATA\\adata.csv", row.names = FALSE)
  }) 
  
  #__________________________________________________________________________________________
  #### SUBREGION: : BOX & WHISKER PLOTS ####
  ...
  output$site.line <- renderPlot({
    ...
    adata3 <- gather(adata2, uni, value, richness:abundance, factor_key=TRUE)
    # View(adata3)
    # write.csv(adata3,'C:\\Users\\KMC00\\OneDrive - CEFAS\\R_PROJECTS\\OneBenthicFaunalChangeTool\\DATA\\adata3.csv')
    ...
    p <- ggplot()+
      ...
      #facet_wrap(~treatment2)+
      #facet_wrap(~uni)+#works
      #facet_grid(treatment2~uni,scales="free_y")+# works 19072022 but not quite right
      facet_grid2(treatment2~uni, scales = "free", independent = "all")
    theme(text = element_text(size=11),legend.title = element_blank(), 
          legend.text=element_text(size=11))#a 
    print(p)
